...
| Portal is offline | Portal keys/environment mismatch, customer policy was not deployed, origin policy rejects the browser, or the hosted service is unavailable. | Run `bun run portal:deploy` against the intended environment and `bun run setup:check`. The application stays offline rather than substituting local data. |
