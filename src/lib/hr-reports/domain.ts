...
function stableMessageContext(
  ...
  if (!isOfficeChannelIdForDay(officeChannelId, officeDayValue)) return null;
...
export function parseHRReportReviewTarget(
  ...
  if (
    ...
    !officeDayValue ||
    !channelSlug ||
    !isOfficeChannelSlug(channelSlug)
  ) {
    return null;
  }
  ...
  return stableMessageContext(officeDayValue, channel.id, messageId);
}
