...
ALTER TABLE "hr_reports" ADD CONSTRAINT "hr_reports_state_check" CHECK ("hr_reports"."state" in ('open', 'dismissed', 'actioned'));--> statement-breakpoint
ALTER TABLE "hr_reports" ADD CONSTRAINT "hr_reports_resolution_check" CHECK (("hr_reports"."state" in ('open', 'actioned') and "hr_reports"."dismissed_by" is null and "hr_reports"."dismissed_at" is null) or ("hr_reports"."state" = 'dismissed' and char_length("hr_reports"."dismissed_by") between 1 and 255 and "hr_reports"."dismissed_at" is not null));--> statement-breakpoint
ALTER TABLE "operator_actions" ADD CONSTRAINT "operator_actions_kind_check" CHECK (("operator_actions"."target_type" = 'hr_report' and "operator_actions"."action" = 'dismissed') or ("operator_actions"."target_type" = 'new_hire' and "operator_actions"."action" = 'sent_home'));--> statement-breakpoint
ALTER TABLE "operator_actions" ADD CONSTRAINT "operator_actions_private_note_check" CHECK (("operator_actions"."action" = 'dismissed' and ("operator_actions"."private_note" is null or char_length("operator_actions"."private_note") between 1 and 1000)) or ("operator_actions"."action" = 'sent_home' and char_length("operator_actions"."private_note") between 1 and 1000));
